...
public class FlappyBird implements ActionListener, MouseListener, KeyListener{
    ...
    public FlappyBird(String user,String pass,String name,int val){
        ...
        try{
            birdImage = ImageIO.read(new File("blue_bird.png"));
            topImage = ImageIO.read(new File("tube_top.png"));
            bottomImage = ImageIO.read(new File("tube_bottom.png"));
            skyImage = ImageIO.read(new File("sky.jpg"));
        }
    ...
    public void paint(Graphics graphics){
        ...
        if(!started) graphics.drawString("Click to start!",200,HEIGHT/2-50);
        graphics.setColor(Color.RED);
        if(gameOver){
            createTextFile = new CreateTextFile(username,password,fileName,highScore);
            graphics.drawString("Game Over!",280,HEIGHT/2);
